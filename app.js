...
function renderAdminPage(adminUser) {
  ...
  document.getElementById("addRewardForm").addEventListener("submit", (e) => {
    e.preventDefault();
    const payload = {
      name: document.getElementById("rewardName").value, description: document.getElementById("rewardDesc").value, pointsRequired: parseInt(document.getElementById("rewardPoints").value, 10), cashRequired: parseInt(document.getElementById("rewardCash").value, 10) || 0, category: document.getElementById("rewardCategory").value, isNew: document.getElementById("rewardIsNew").checked, adminPhone: adminUser.phone,
    };
    apiCall("addReward", payload).then(() => { Swal.fire("สำเร็จ", "เพิ่มของรางวัลใหม่เข้าระบบเรียบร้อยแล้ว", "success"); document.getElementById("addRewardForm").reset(); }).catch((err) => console.error(err));
  });
}
